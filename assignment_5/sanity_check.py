...
def question_1f_sanity_check(highway):
    ...
    Arguments:
        highway {nn.Module} -- should return a tensor with shape (e_word)
    """
    print ("-"*80)
    print("Running Sanity Check for Question 1f: Highway net")
    print ("-"*80)

    x_conv = torch.randn((BATCH_SIZE, EMBED_SIZE))
    output_expected_size = [BATCH_SIZE, EMBED_SIZE]
    x_word_emb = highway(x_conv)
    assert(list(x_word_emb.size()) == output_expected_size), "output shape is incorrect: it should be: \n {} but is :\n{}".format(output_expected_size, list(x_word_emb.size()))
    print("Sanity Check Passed for Question 1f: Highway net!")
    print("-"*80)
...
def question_1g_sanity_check():
    ...
    Arguments:
        cnn {nn.Module} -- should return a tensor with shape (batch_size, e_word)
    """
    print ("-"*80)
    print("Running Sanity Check for Question 1g: CNN net")
    print ("-"*80)
    max_word_length = 3
    e_char = 5
    cnn = CNN(EMBED_SIZE, e_char)
    x_reshape = torch.randn((BATCH_SIZE, e_char, max_word_length))
    output_expected_size = [BATCH_SIZE, EMBED_SIZE]
    x_conv_out = cnn(x_reshape)
    assert(list(x_conv_out.size()) == output_expected_size), "output shape is incorrect: it should be: \n {} but is :\n{}".format(output_expected_size, list(x_word_emb.size()))
    print("Sanity Check Passed for Question 1f: CNN net!")
    print("-"*80)
