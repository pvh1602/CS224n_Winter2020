...
class NMT(nn.Module):
    ...
    def __init__(self, word_embed_size, hidden_size, vocab, dropout_rate=0.3, no_char_decoder=False):
        ...
        self.encoder = nn.LSTM(self.model_embeddings_source.word_embed_size, self.hidden_size, bidirectional= True)
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        target_padded = self.vocab.tgt.to_input_tensor(target, self.device) #(tgt_len, batch_size)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_state[0], dim = 2))      #b x scr_len x 1
        ...
        a_t = torch.bmm(torch.unsqueeze(alpha_t, dim = 1), enc_hiddens) # b x 1 x 2h
        a_t = torch.squeeze(a_t, dim = 1)   # b x 2h
